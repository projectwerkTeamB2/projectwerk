...
{
    public class StripRepository_OG : IStripRepository
    {
        ...
        public StripRepository_OG(DbProviderFactory sqlFactory)
        {
        ...
        public void allesWegSchijvenNaarDataBank(List<Strip> strips)
        {
            ...
            Dictionary<int, AuteurRepository> dictAuteurs = new Dictionary<int, AuteurRepository>();
            ...
            {
                foreach (AuteurRepository a in s.Auteurs)
                {
            ...
            using (DbCommand command = connection.CreateCommand())
            {
                ...
                try
                {
                    ...
                    foreach (KeyValuePair<int, AuteurRepository> dA in dictAuteurs)
                    {
        ...
        public IEnumerable<Strip> FindAll_ByAuteur(AuteurRepository auteur)
        {
            DbConnection connection = getConnection();
            ...
            using (DbCommand command = connection.CreateCommand())
            {
                ...
                try
                {
                    ...
                    while (data.Read())
                    {
                        if (!listStrips.Any())
                        {
                            List<AuteurRepository> list = new List<AuteurRepository>();
                            AuteurRepository auteur = new AuteurRepository((int)data[2], (string)data[3]);
                            list.Add(auteur);
                        ...
                        {
                            listStrips[teller].Auteurs.Add(new AuteurRepository((int)data[2], (string)data[3]));
        ...
        public IEnumerable<Strip> FindAll_strip()
        {
            ...
            using (DbCommand command = connection.CreateCommand())
            {
                ...
                try
                {
                    ...
                    {
                        List<AuteurRepository> auteurList = new List<AuteurRepository>();
                        ...
                        {
                            listStrip[listStrip.Count - 1].Auteurs.Add(new AuteurRepository((int)datareader["Auteur_Id"], (string)datareader[10]));
                        }
                        else if (vorigeStripTitel != (string)datareader["Titel"])
                        {
                            auteurList.Add(new AuteurRepository((int)datareader["Auteur_Id"], (string)datareader[10]));
        ...
        public AuteurRepository GetAuteur_fromName(string naam)
        {
            ...
            using (DbCommand command = connection.CreateCommand())
            {
                ...
                try
                {
                    ...
                    IDataReader datareader = command.ExecuteReader();
                    datareader.Read();
                    AuteurRepository auteur = new AuteurRepository((int)datareader["Id"], (string)datareader["Name"]);
